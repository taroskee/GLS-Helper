...
def test_save_nodes_batch_performance(tmp_path, benchmark):
    ...
    with closing(sqlite3.connect(db_path)) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT count(*) FROM nodes")
        assert cursor.fetchone()[0] >= node_count
...
def test_update_edges_delay_batch_updates_existing_records(tmp_path):
    ...
    Note: The update logic maps SDF destination pin to DB source pin.
    """
    # Arrange
    ...
    initial_edges = [
        Edge(
            src_node="u2.A", dst_node="net_internal_1", delay_rise=0.0, delay_fall=0.0
        ),
        Edge(
            src_node="u3.B", dst_node="net_internal_2", delay_rise=0.0, delay_fall=0.0
        ),
        Edge(
            src_node="u4.C", dst_node="net_internal_3", delay_rise=0.0, delay_fall=0.0
        ),
    ]
    ...
    with closing(sqlite3.connect(db_path)) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT src, delay_rise, delay_fall FROM edges ORDER BY src")
